// Blackwell MXFP8 → BF16 GEMM. PIMPL'd so the heavy CUTLASS templates stay
// out of the header. Mirrors fp4_gemm.cu; the only substantive differences are
// the operand element types (mx_float8_t<e4m3> vs nv_float4_t<e2m1>), the
// alignment (16 vs 32), and the 1-byte-per-element operand sizing.
...
struct FP8Gemm::Impl { const char* err = "CUTLASS built without SM120/SM121 block-scaled support"; };
FP8Gemm::FP8Gemm(int, int, int) : impl_(std::make_unique<Impl>()) {}
FP8Gemm::~FP8Gemm() = default;
FP8Gemm::FP8Gemm(FP8Gemm&&) noexcept = default;
FP8Gemm& FP8Gemm::operator=(FP8Gemm&&) noexcept = default;
bool FP8Gemm::ok() const { return false; }
const char* FP8Gemm::last_error() const { return impl_ ? impl_->err : "no impl"; }
int FP8Gemm::m() const { return 0; }
int FP8Gemm::n() const { return 0; }
int FP8Gemm::k() const { return 0; }
size_t FP8Gemm::a_size_bytes() const { return 0; }
size_t FP8Gemm::b_size_bytes() const { return 0; }
size_t FP8Gemm::c_size_bytes() const { return 0; }
size_t FP8Gemm::d_size_bytes() const { return 0; }
size_t FP8Gemm::sfa_size_bytes() const { return 0; }
size_t FP8Gemm::sfb_size_bytes() const { return 0; }
size_t FP8Gemm::workspace_size_bytes() const { return 0; }
bool FP8Gemm::run(const void*, const void*, const void*, const void*,
                  const void*, void*, void*, float, float, cudaStream_t) {
    return false;
}

FP8GemmResult run_fp8_gemm_smoke(int, int, int, int) {
    FP8GemmResult r;
    r.error_msg = "CUTLASS built without SM120/SM121 block-scaled support";
    return r;
}
...
// --- Type configuration: BOTH operands MXFP8 (mirrors example 79c's A). ---
...
FP8Gemm::FP8Gemm(int m, int n, int k) : impl_(std::make_unique<Impl>()) {
    if (m % 128 != 0 || n % 128 != 0 || k % 128 != 0) {
        impl_->err = "m,n,k must be multiples of 128";
        return;
    }
    impl_->m = m;
    impl_->n = n;
...
bool        FP8Gemm::ok()         const { return impl_ && impl_->valid; }
const char* FP8Gemm::last_error() const {
    return impl_ ? (impl_->err.empty() ? "" : impl_->err.c_str()) : "no impl";
}
int FP8Gemm::m() const { return impl_->m; }
int FP8Gemm::n() const { return impl_->n; }
int FP8Gemm::k() const { return impl_->k; }

size_t FP8Gemm::a_size_bytes() const { return static_cast<size_t>(impl_->m) * impl_->k; }
size_t FP8Gemm::b_size_bytes() const { return static_cast<size_t>(impl_->n) * impl_->k; }
size_t FP8Gemm::c_size_bytes() const {
    return static_cast<size_t>(impl_->m) * impl_->n * sizeof(cutlass::bfloat16_t);
}
size_t FP8Gemm::d_size_bytes()         const { return c_size_bytes(); }
size_t FP8Gemm::sfa_size_bytes()       const { return impl_->sfa_bytes; }
size_t FP8Gemm::sfb_size_bytes()       const { return impl_->sfb_bytes; }
size_t FP8Gemm::workspace_size_bytes() const { return impl_->workspace_bytes; }

bool FP8Gemm::run(const void* A, const void* SFA,
...
// ----------------------------------------------------------------------------
// Smoke test built on top of FP8Gemm.
// ----------------------------------------------------------------------------
...
FP8GemmResult run_fp8_gemm_smoke(int m, int n, int k, int iterations) {
    FP8GemmResult res;
